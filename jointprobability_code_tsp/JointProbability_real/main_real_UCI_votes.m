...
%Test Variables
[Data,I]    = LoadDataset_voters();
[rows, col] = size(Data);
N           = length(I);
F_list_SPA=[2,3,4,5,6];
F_list_SPA1=[2,3,4,5,6];
F_list_Tensor=[2,3,4,5,6];
...
for s = 1:n_sim
    ...
    [trainIndices,validIndices,testIndices] = dividerand(rows,0.5,0.2,0.3);
...
%                A_est{i}=G;
%             end
            opts2.A0=A_est;opts2.l0=l0;%l_SPA{fi};
            opts2.marg    = marg;
            opts2.computeCost=1;
            opts2.computeMSE=0;
...
rows = {'Accuracy';'STD_Accuracy';'time(s)'};
 ...
 T = table(CNMF_SPA,CNMF_OPT,SPA_EM,CTD,CTD_EM,MLE_AD,MLE_EM,SVM,LINEAR_REGRESSION,NNET,SVM_RBF,NAIVE_BAYES,...
    'RowNames',rows);

writetable(T,'table_votes.csv','WriteRowNames',true)
save data_votes