...
%     grad_lambda(isnan(grad_lambda))=0;    
    ...
    [lambda_new,cost_new]  = armijo_rule(lambda_new,Y,Q,grad_lambda,beta,sigma,step,cost_prev);
    c = [c; cost_new];
    cost_prev = c(end);
...
function [lambda,cost_new] = armijo_rule(lambda_old,Y,Q,grad_lambda,beta,sigma,step,cost_prev)
lambda_new = lambda_old .* exp(-step*grad_lambda);
lambda_new = lambda_new/sum(lambda_new );
cost_new   = cost(lambda_new,Y,Q);
