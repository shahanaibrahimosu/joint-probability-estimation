...
Movie_ids = Movie_ids_most_popular_romanc_30;
...
%         R = F_tensor_list(f);
%         fprintf('Rank used: %d \n',R)
%         [A0,l0]   = gen_PMF_factors(I,R);
%         l0  = 1/R*ones(R,1);
%         [A,lambda,Out] = run_sweeps(Y,A0,l0,opts);
%         temp=cumsum(Out.time_instants);
%         time_mle(sim,f)=temp(end);
%         [rmse_valid_t(f),mae_valid_t(f),pr_rating,num_nan] = complete_rating_matrix(TrainSet,P3,r3,c3,A,lambda);
%         A_MLE_t{f}=A;
%         l_MLE_t{f}=lambda;
%     end
...
%     A_sel=A_MLE_t{ind};
%     l_sel=l_MLE_t{ind};
%     [rmse_mle(sim),mae_mle(sim),pr_rating,num_nan] = complete_rating_matrix(TrainSet,P4,r4,c4,A_sel,l_sel);
...
%             a=tic;
%             %Nround = 10; %number of EM iterations
%             [A_est_em_mle,l_est_em_mle] = run_EM(A_MLE_t{f},l_MLE_t{f},Z,Nround);
%             b=toc(a);
%             time_mle_em(sim,f)=b;
%             [rmse_valid_mle_em(iter,f),mae_valid_mle_em(iter,f),pr_rating,num_nan] = complete_rating_matrix(TrainSet,P3,r3,c3,A_est_em_ctd,l_est_em_ctd);
%             A_mle_em{iter,f}=A_est_em_mle;
%             l_mle_em{iter,f}=l_est_em_mle;
%             iter=iter+1;
%         end        
%      end
%     max_val=max(rmse_valid_mle_em,[],'all');
%     [ind1,ind2]=find(rmse_valid_mle_em==max_val);
%     F_sel=F_tensor_list(ind2(1));
% %     A_sel=A_em{ind1(1),ind2(1)};
% %     l_sel=l_em{ind1(1),ind2(1)};
%      [A_sel,l_sel] = run_EM(A_MLE_t{ind2(1)},l_MLE_t{ind2(1)},Z,Nround);
%     [rmse_mle_em(sim),mae_mle_em(sim),pr_rating,num_nan] = complete_rating_matrix(TrainSet,P4,r4,c4,A_sel,l_sel);
% 
...
rows = {'RMSE';'MAE';'STD-RMSE';'STD-MAE';'time(s)'};
 ...
 %MLE_AD = [nanmean(rmse_mle);nanmean(mae_mle);std(rmse_mle);std(mae_mle);mean(time_mle,"all")];
 %MLE_EM = [nanmean(rmse_mle_em);nanmean(mae_mle_em);std(rmse_mle_em);std(mae_mle_em);mean(time_mle_em,"all")];
 ...
 T = table(CNMF_SPA,SPA_EM,CTD,CTD_EM,BMF,Global_average,User_average,Movie_average,...
    'RowNames',rows);
writetable(T,'table_romance.csv','WriteRowNames',true)
save data_romance
% disp('########Results##############');
